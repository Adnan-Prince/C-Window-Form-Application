...
namespace Al_Ain_DataEntry
{
    public partial class DataEntry : Form
    {
        ...
        {
            DataTable dt = dal.DALGetBranchName();
            combobranchname.DisplayMember = "BranchName";
            combobranchname.DroppedDown = true;

            List<string> list = new List<string>();
            foreach (DataRow row in dt.Rows)
            {
                list.Add(row.Field<string>("BranchName"));
            }
            this.combobranchname.Items.AddRange(list.ToArray<string>());
